...
/* Additional heders from <mongoose-os-libs>/rpc-common/ */
...
static void wifi_utils_disconnect_wifi_cb(struct mg_rpc_request_info *ri, void *cb_arg,
	...
	if (strcmp(ri->ch->get_type(ri->ch), "HTTP")==0) {
		...
		ip_src = mg_rpc_channel_tcp_get_info(chd->nc);
	} else if (strcmp(ri->ch->get_type(ri->ch), "WS_in")==0) {
		struct mg_rpc_channel_ws_data *chd = (struct mg_rpc_channel_ws_data *) ri->ch->channel_data;
		ip_src = mg_rpc_channel_tcp_get_info(chd->nc);
	}
	...
	if (ip_src != NULL) {
		...
		mg_rpc_send_responsef(ri, "{error: \"%s\"}", "Can disconnect only from RPC over HTTP or WS");
	}
